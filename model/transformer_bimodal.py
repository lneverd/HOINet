...
class Transformer_Encoder(nn.Module):
    ...
    def forward(self, src, src_pos=None, src_mod=None, key_padding_mask=None, attn_mask=None, verbose=False):
        ...
        memory, list_attn_maps = self.encoder(src, src_attn_mask=attn_mask, src_key_padding_mask=key_padding_mask,
                                              src_pos=src_pos, src_mod=src_mod, verbose=verbose)
    ...
    def forward(self, src,
        ...
        for layer in self.layers:
            output, attn_map = layer(output, src_attn_mask=src_attn_mask,
                                     src_key_padding_mask=src_key_padding_mask, src_pos=src_pos, src_mod=src_mod,
                                     verbose=verbose)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        src2 ,attn_map= self.self_attn(q, k, value=src, attn_mask=src_attn_mask,
                              key_padding_mask=src_key_padding_mask) #注释

        src = src + self.dropout1(src2)
        src = self.norm1(src)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src)))) #保留
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        x_pe = self.pe[:, :x.size(1)].detach()
        x_pe = x_pe.unsqueeze(0)  # 增加一个batch维度
        x_pe = x_pe.repeat(x.size(0), 1, 1)  # 沿着batch维度复制
        return x_pe
