...
def valid_crop_resize(data_numpy,rgb_data,valid_frame_num,p_interval,window):
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
    time, channel, height, width = rgb_data.shape
    begin = 0
    end = valid_frame_num
    ...
    if len(p_interval) == 1:
        ...
        rgb_data = rgb_data[begin+bias:end-bias, :, :, :]# center_crop
        ...
        rgb_data = rgb_data[begin + bias:begin + bias + cropped_length, :, :, :]
        if data.shape[1] == 0:
    ...
    rgb_data = rgb_data.permute(1,2,3,0).contiguous().view(channel*height*width, cropped_length)
    data = data[None, None, :, :]
    rgb_data = rgb_data[None, None, :, :]
    ...
    rgb_data = F.interpolate(rgb_data, size=(channel*height*width, window), mode='bilinear', align_corners=False).squeeze()
    data = data.contiguous().view(C, V, M, window).permute(0, 3, 1, 2).contiguous().numpy()
    rgb_data = rgb_data.contiguous().view(channel, height, width, window).permute(3, 0, 1, 2).contiguous()
