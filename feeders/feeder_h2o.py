...
class Feeder(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=1, split='train', random_choose=False, random_shift=False,
                 ...
                 bone=False, vel=False, entity_rearrangement=False,use_rgb=False,img_size=224):#2 hands not eneityrearrange
        """
        data_path:
        label_path:
    ...
    def get_H2O(self, root_dir, split='train'):
        ...
        obj_labels = torch.load(os.path.join(root_dir, split, 'obj_labels.pth'))
        ...
        for i in range(ground_truth.size(0)):
            ground_truth[i] = ground_truth[i] - 1 #为什么从0开始???
        for i in range(obj_labels.size(0)):
            obj_labels[i] = obj_labels[i]-1 #obj label-1
        
        return data, ground_truth,obj_labels,rgb_paths


    def load_data(self):
        # N, C, T, V, M
        self.data, self.label, self.obj_labels,self.rgb_paths = self.get_H2O(root_dir=self.data_path, split=self.split)
        if self.split == 'train':
    ...
    def __getitem__(self, index):
        ...
        data_numpy,rgb_data = tools.valid_crop_resize(data_numpy,rgb_data, valid_frame_num, self.p_interval, self.window_size)
        if self.random_rot:
        ...
        return data_numpy,rgb_data, label,obj_label,index
...
class Feeder_Test(Dataset):
    ...
    def get_H2O(self, root_dir, split='test'):
        ...
        for i in range(obj_labels.size(0)):
            obj_labels[i] = obj_labels[i]-1 #obj label-1
        return data,obj_labels,rgb_paths


    def load_data(self):
        # N, C, T, V, M
        self.data,self.obj_labels,self.rgb_paths = self.get_H2O(root_dir=self.data_path, split=self.split)
        if self.split == 'test':
    ...
    def __getitem__(self, index):
        ...
        return data_numpy,rgb_data, obj_label,index