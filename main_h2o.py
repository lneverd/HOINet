...
def get_parser():
    ...
    parser.add_argument('--work_dir', default='./exp/h2o', help='the work folder for storing results')
    parser.add_argument('--config', default='./config/h2o/h2o.yaml', help='path to the configuration file')
    ...
    # feeder
    parser.add_argument('--feeder', default='feeders.feeder_h2o.Feeder', help='data loader will be used')
    ...
    parser.add_argument('--weights', default='D:/Downloads/h2o (7).pt', help='the weights for model testing')
...
def accuracy(output, target, topk=(1,)):
    ...
    return res
def get_total_loss(obj_loss,verb_loss,action_loss,l1_loss,noun_loss_weight,verb_loss_weight,action_loss_weight,feat_loss_weight):
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        self.action2verb_tensor = torch.tensor([
            0, 0, 0, 0, 0, 0, 0, 0,  # action 0-7 -> grab (verb 0)
            1, 1, 1, 1, 1, 1, 1, 1,  # action 8-15 -> place (verb 1)
            2, 2, 2,  # action 16-18 -> open (verb 2)
            3, 3, 3,  # action 19-21 -> close (verb 3)
            4,  # action 22 -> pour (verb 4)
            5, 5, 5, 5,  # action 23-26 -> take out (verb 5)
            6, 6, 6,  # action 27-29 -> put in (verb 6)
            7, 7,  # action 30-31 -> apply (verb 7)
            8, 8,  # action 32-33 -> read (verb 8)
            9,  # action 34 -> spray (verb 9)
            10  # action 35 -> squeeze (verb 10)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch, (data,rgb_data ,label,object_label, sample) in enumerate(tqdm(self.data_loader['train'], desc="Training", ncols=100)):
            self.global_step += 1
            with torch.no_grad():
                data = data.float().cuda(self.output_device)
                label = label.long().cuda(self.output_device)
                object_label = object_label.long().cuda(self.output_device)
            ...
            loss = get_total_loss(obj_loss,verb_loss,action_loss,l1_loss,self.noun_loss_weight,self.verb_loss_weight,self.action_loss_weight,self.feat_loss_weight)
            # backward
        ...
        self.print_log('training: epoch: {}, loss: {:.4f}, top1: {:.2f}%, lr: {:.6f}, obj_loss: {:.4f}, verb_loss:{:.4f},l1_loss:{:.4f}, obj_acc:{:.4f}%, verb_acc:{:.4f}%'.format(
            epoch + 1, losses.avg, top1.avg, self.lr, obj_losses.avg, verb_losses.avg,l1_losses.avg, obj_prec_top1.avg, verb_prec_top1.avg))
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            label_list = []
            pred_list = []
            for batch, (data,rgb_data,label,object_label, sampie) in enumerate(tqdm(self.data_loader[ln], desc="Evaluating", ncols=100)):
            ...
            self.print_log('evaluating: loss: {:.4f}, top1: {:.2f}%, best_acc: {:.2f}%,obj_loss: {:.4f},verb_loss:{:.4f},l1_loss:{:.4f},obj_acc:{:.4f}%, verb_acc:{:.4f}%'.format
                           (losses.avg, top1.avg, self.best_acc,obj_losses.avg,verb_losses.avg,l1_losses.avg,obj_prec_top1.avg, verb_prec_top1.avg))
    ...
    def h2o_get_results(self, loader_name=['test'], result_file=None):
        ...
        self.model.eval()
        for ln in loader_name:
            for batch, (data,rgb_data,object_label,index) in enumerate(tqdm(self.data_loader[ln], desc="Evaluating", ncols=100)):
                with torch.no_grad():
                    data = data.float().cuda(self.output_device)
                    # object_label = object_label.long().cuda(self.output_device)
                    rgb_data = rgb_data.cuda(self.output_device)
    ...
    def asb_get_results(self, loader_name=['test'], result_file=None):
        ...
        self.model.eval()
        for ln in loader_name:
            for batch, (data,rgb_data,obj_label,index) in enumerate(tqdm(self.data_loader[ln], desc="Evaluating", ncols=100)):
                with torch.no_grad():
                    data = data.float().cuda(self.output_device)
                    # object_label = object_label.long().cuda(self.output_device)
                    rgb_data = rgb_data.cuda(self.output_device)
